...
describe("Sync API Endpoints", () => {
  ...
  describe("POST /api/sync/upload", () => {
    it("stores uploaded blob and returns key + timestamp", async () => {
      ...
      const request = makeRequest("/api/sync/upload", {
        method: "POST",
        headers: {
  ...
  describe("GET /api/sync/list", () => {
    it("returns changesets for a vault", async () => {
      ...
      const request = makeRequest(
        `/api/sync/list?vaultPath=${TEST_VAULT}`
      );
      ...
      expect(json.changesets[0].key).toBe(`${TEST_VAULT}/1000-device1.bin`);
    });
    ...
    it("filters changesets with after parameter", async () => {
      ...
      const request = makeRequest(
        `/api/sync/list?vaultPath=${TEST_VAULT}&after=1000`
      );
      const response = await listGET(makeAPIEvent(request));
      const json = await response.json();
    ...
    it("returns empty array for nonexistent vault", async () => {
      const request = makeRequest(
        `/api/sync/list?vaultPath=nonexistent-vault`
      );
      const response = await listGET(makeAPIEvent(request));
      const json = await response.json();
  ...
  describe("GET /api/sync/download/:key", () => {
    it("returns the encrypted blob", async () => {
      ...
      const key = `${TEST_VAULT}/1000-device1.bin`;
      const request = makeRequest(`/api/sync/download/${key}`);
    ...
    it("returns 404 for nonexistent key", async () => {
      const key = `${TEST_VAULT}/9999-nodevice.bin`;
      const request = makeRequest(`/api/sync/download/${key}`);
      const response = await downloadGET(makeAPIEvent(request, { key }));
  ...
  describe("Full sync cycle", () => {
    it("upload → list → download returns same data", async () => {
      ...
      // Upload
      const uploadReq = makeRequest("/api/sync/upload", {
        method: "POST",
        headers: {
          "X-Vault-Path": TEST_VAULT,
      ...
      // Download
      const dlReq = makeRequest(`/api/sync/download/${key}`);
